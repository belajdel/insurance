...
@Component({
  selector: 'app-update-assurance-sante-personne',
  templateUrl: './update-assurance-sante-personne.component.html',
  styleUrl: './update-assurance-sante-personne.component.css'
})
export class UpdateAssuranceSantePersonneComponent {
  verife: boolean = false
  protected assurancePersonne : SantePersonne = history.state?.assurance
  ...
  /*fillData(){
    this.assurancePersonneService.getSantePersonneById(this.activatedRoute.snapshot.params['id']).subscribe((Response)=>{
      this.assurancePersonne=Response;
      this.startDate=String(this.assurancePersonne.startDate)
      this.endDate=String(this.assurancePersonne.endDate)
      this.periode=this.assurancePersonne.periode_assurance
    })
  }*/
  constructor(protected updateSanteComponent: UpdateSanteComponent,private activatedRoute:ActivatedRoute, private tokenStorageService: TokenStorageService, private assurancePersonneService: SantePersonneService, private router: Router, private http: HttpClient) {
  }
  ...
  ngOnInit(): void {
    ...
    this.endDate=String(this.assurancePersonne.endDate)
    this.periode=this.assurancePersonne.periode_assurance
    this.ageInput=String(this.assurancePersonne.date_naissance)
  ...
  CalculTaxe() {
    ...
    if (this.updateSanteComponent.assurance_type == 'صحي فرد') {
      ...
      this.assurancePersonne.taxe3 = (this.assurancePersonne.initial / 100)
  ...
  OnRecuper() {
    ...
    this.assurancePersonneService.UpdateSantePersonne(this.assurancePersonne).subscribe((Response) => {
