...
import { Travel } from 'src/app/public/models/travel.model';
...
export class TravelComponent {
  ...
  protected verife:boolean=false
  protected travel=new Travel();
  ...
  ngOnInit(): void {
    //this.age=new Date()
    //this.ageInput=this.getFormattedDate();
    this.startDate=this.getFormattedDate();
    this.birthDate=this.getFormattedDate();
    this.getcountry();
    this.travel.direction="إختر الوجهة"
    this.travel.nationalite="إختر الجنسية"
    this.travel.m_f='إختر الجنس'
  }
  ...
  years: string[] = [];
  constructor(private travelService:TravelService,private router: Router,private http: HttpClient,private tokenStorageService:TokenStorageService) {
...
calculeAge(){
    ...
    const dateNaissance = new Date(this.birthDate);
 ...
 // word wide
  ...
  OnRecuper(){
    ...
    this.travelService.createTravel(this.travel).subscribe((Response)=>{
